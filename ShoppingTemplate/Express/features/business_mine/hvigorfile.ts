...
function writeHostIndex(taskContext: any, sourcePath: string, targetPath: string): void {
  ...
  const mainEtsPath = path.resolve(taskContext.modulePath, 'src/main/ets');
  const mainEtsImportPath = normalizeToPosix(path.relative(targetDir, mainEtsPath));
...
const replaceOhosTestIndexPlugin: HvigorPlugin = {
  pluginId: 'business_mine_replace_ohos_test_index',
  apply(node) {
    hvigor.nodesEvaluated(() => {
      ...
      node.registerTask({
        name: 'CopyBusinessMineOhosTestHostIndex',
